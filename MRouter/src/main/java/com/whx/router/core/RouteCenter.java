...
import static com.whx.router.core.Constants.ROUTER_CURRENT_PATH;
import static com.whx.router.core.Constants.ROUTER_RAW_URI;
...
import com.whx.router.exception.NoFoundRouteException;
...
import com.whx.router.model.ParamMeta;
...
public class RouteCenter {
    ...
    public static void addRouterGroup(String group, IRouteModuleGroup routeModuleGroup) {
        ...
        MRouter.logger.info(null, "[addRouterGroup] Add a route group[" + group + "] dynamically");
    }
    ...
    public static CardMeta getCardMeta(Card card) throws NoFoundRouteException {
        ...
        if (cardMeta == null) {
            if (!Warehouse.routeGroups.containsKey(card.getGroup())) {
                ...
                try {
                    if (MRouter.isDebug()) {
                        MRouter.logger.debug(null, "[getCardMeta] The group [" + card.getGroup() + "] starts loading, trigger by [" + card.getPath() + "]");
                    }
                    ...
                    if (MRouter.isDebug()) {
                        MRouter.logger.debug(null, "[getCardMeta] The group [" + card.getGroup() + "] has already been loaded, trigger by [" + card.getPath() + "]");
                    }
                } catch (Exception e) {
            ...
            MRouter.logger.info(null, "[getCardMeta] " + cardMeta);
        }
        return cardMeta;
    }
    ...
    public static void addCardMeta(CardMeta cardMeta) {
        ...
        MRouter.logger.debug(null, "[addCardMeta] size:" + Warehouse.routes.size() + ", commit:" + cardMeta);
    }
    ...
    public static <T> void inject(T target, Intent intent, Bundle bundle, boolean isCheck) throws ParamException {
        MRouter.logger.debug(null, "[inject] Auto Inject Start!");

        try {
            bundle = getBundle(target, intent, bundle);
        } catch (Exception e) {
            MRouter.logger.warning(null, "inject() " + e.getMessage());
            return;
        }

        String path = getCurrentPath(bundle);
        if (TextUtils.isEmpty(path)) {
            MRouter.logger.warning(null, "[inject] The " + ROUTER_CURRENT_PATH + " parameter was not found in the intent");
            return;
        }

        CardMeta cardMeta = MRouter.getInstance().build(path).getCardMeta();
        if (cardMeta != null) {
            ...
            for (Map.Entry<String, ParamMeta> entry : paramsType.entrySet()) {
                ...
                MRouter.logger.debug(null, "[inject] " + paramName + ":" + value);
                try {
                    ...
                    if (ParamType.Object == entry.getValue().getType() && value instanceof String) {
                        if (jsonService == null) {
                            jsonService = MRouter.getInstance().getService(IJsonService.class);
                        }
                        if (jsonService == null) {
        ...
        MRouter.logger.debug(null, "[inject] Auto Inject End!");
    }
