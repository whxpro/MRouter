...
public class EventCenter {
    ...
    public static <T> void registerEvent(LifecycleOwner owner, Class<T> type, boolean isForever, @NonNull Observer<T> observer) {
        ...
        if (TextUtils.isEmpty(path)) {
            MRouter.logger.error(null, "[registerEvent] The " + ROUTER_CURRENT_PATH + " parameter was not found in the intent");
            return;
        }
        ...
        if (liveData != null) {
            ...
            MRouter.logger.error(null, "[registerEvent] LiveData is empty??");
        }
    }
    ...
    public static <T> void unRegisterEvent(LifecycleOwner owner, Class<T> type, Observer<T> observer) {
        ...
        if (Warehouse.events.containsKey(key)) {
            ...
            MRouter.logger.warning(null, "[unRegisterEvent] No observer was found for this event");
        }
    }
    ...
    public static <T> void postEvent(String path, T value) {
        ...
        for (Map.Entry<String, MutableLiveData> entry : Warehouse.events.entrySet()) {
            if (entry.getKey().startsWith(key)) {
                ...
                if (liveData != null) {
                    MRouter.logger.info(null, "[postEvent] send path[" + entry.getKey() + "] value[" + value + "]");
                    if (Looper.myLooper() == Looper.getMainLooper()) {
                    ...
                    MRouter.logger.error(null, "[postEvent] LiveData is empty??");
                }
            }
        }
        if (!isFound) {
            MRouter.logger.warning(null, "[postEvent] No observer was found for this event");
        }
    }
