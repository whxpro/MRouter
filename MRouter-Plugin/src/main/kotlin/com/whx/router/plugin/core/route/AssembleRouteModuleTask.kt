package com.whx.router.plugin.core.route

import com.whx.router.plugin.utils.Constants
import com.whx.router.plugin.utils.Constants.ROUTE_MODULE_INJECT_CLASS_NAME
import com.whx.router.plugin.utils.Constants.ROUTE_MODULE_NAME_SUFFIX
import com.whx.router.plugin.utils.Constants.ROUTE_MODULE_SCAN_TARGET_INJECT_PACKAGE_NAME
import com.whx.router.plugin.utils.Constants._CLASS
...
abstract class AssembleRouteModuleTask : DefaultTask() {
    ...
    private val routeModuleClassList = ArrayList<String>()
    ...
    fun taskAction() {
        ...
        if (originInject == null) {
            Logger.e(TAG, "Can not find MRouter inject point, Do you import [MRouter-Api]???")
            return
        }
...
//            Logger.i("Scan to directory [${directory.asFile.absolutePath}]")
            directory.asFile.walk().forEach { file ->
                if (file.isFile) {
                    if (file.name.endsWith(ROUTE_MODULE_NAME_SUFFIX + _CLASS)) {
                        Logger.i(TAG, "Scan to class [${file.name}] be from directory [${directory.asFile.absolutePath}]")
                        routeModuleClassList.add(file.name)
                    }
...
//            Logger.i("Scan to jar [${file.asFile.absolutePath}]")
            ...
            while (enumeration.hasMoreElements()) {
                ...
                try {
                    ...
                    if (Constants.dotToSlash(ROUTE_MODULE_INJECT_CLASS_NAME) + _CLASS == entryName) {
                        Logger.i(TAG, "Find the inject class [$entryName]")
                        jarFile.getInputStream(jarEntry).use { inputs ->
                            originInject = inputs.readAllBytes()
                        }
                    } else {
                        val startsWith = entryName.startsWith(Constants.dotToSlash(ROUTE_MODULE_SCAN_TARGET_INJECT_PACKAGE_NAME))
                        val endsWith = entryName.endsWith(Constants.dotToSlash(ROUTE_MODULE_NAME_SUFFIX) + _CLASS)
            ...
            routeModuleClassList.addAll(tempList)
            jarFile.close()
        }
    }

    private fun injectCode() {
        val resultByteArray = AssembleRouteModuleCodeInjector(routeModuleClassList).execute(ByteArrayInputStream(originInject))
        jarOutput!!.putNextEntry(JarEntry(Constants.dotToSlash(ROUTE_MODULE_INJECT_CLASS_NAME) + _CLASS))
